 ...
 * Node initialization + Guardian selection.
 * ============================================================
 */
...
async function handleStart(interaction) {
  ...
  if (existing) {
    const g = GUARDIANS[existing.guardian];
      ...
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            ...
            `Guardian: ${g?.emoji} **${g?.name}**\n` +
            `Stage: **${db.getStageName(existing.stage)}**\n\n` +
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: 'Your choice is permanent. Choose the Citadel you want to build.' });

  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('guardian_nova') .setLabel('🦉 Choose Nova') .setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId('guardian_tarin').setLabel('🐘 Choose Tarin').setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId('guardian_raxa') .setLabel('🐅 Choose Raxa') .setStyle(ButtonStyle.Danger),
  );
...
async function handleGuardianSelect(interaction) {
  ...
  const existing = await db.getPlayer(userId);
  if (existing) {
    return interaction.followUp({ content: 'Your Node is already initialized.', ephemeral: true });
  }

  await db.createPlayer(userId, username, guardianKey);
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      { name: 'GUARDIAN BONDED', value: `${guardian.emoji} **${guardian.name}**`, inline: true },
      { name: 'STAGE',           value: '⬛ Dormant Node',                          inline: true },
      { name: 'STARTING POWER',  value: '🔋 5 / 10',                               inline: true },
      { name: 'YOUR BONUSES',    value: `\`${guardian.bonus_1}\`\n\`${guardian.bonus_2}\`` },
      {
        name:  '━━━━━━━━━━━━━━━━',
        value: '> Your Citadel begins as a **Dormant Node**.\n' +
    ...
    .setFooter({ text: 'Citadel Ascension  |  Your Citadel awaits.' });

  return interaction.editReply({ embeds: [embed], components: [] });
}

module.exports = { handleStart, handleGuardianSelect };