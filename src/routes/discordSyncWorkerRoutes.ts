...
router.get("/", async (_req: Request, res: Response) => {
  try {
    ...
    console.error("GET /discord-sync-worker failed:", error);
    return res.status(500).json({
      error: "Failed to build role sync payloads",
    });
  }
});
...
router.get("/:userId", async (req: Request, res: Response) => {
  try {
    ...
    console.error("GET /discord-sync-worker/:userId failed:", error);
    return res.status(500).json({
      error: "Failed to build role sync payload",
    });
  }
});
...
router.post("/:userId/mark-synced", async (req: Request, res: Response) => {
  try {
    ...
    console.error(
      "POST /discord-sync-worker/:userId/mark-synced failed:",
      error
    );
    return res.status(500).json({
      error: "Failed to mark role sync",
    });
  }
});
